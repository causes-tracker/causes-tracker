...
/// never materializes the full result set.  No ordering is imposed — Postgres
/// returns rows in heap order via a sequential scan, giving O(1) time to
/// first batch regardless of table size.
///
...
mod tests {
    ...
    async fn list_projects_returns_all(pool: sqlx::PgPool) {
        ...
        let mut projects = collect_all_projects(&pool, &session).await;
