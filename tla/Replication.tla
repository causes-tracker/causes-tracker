--------------------------- MODULE Replication ---------------------------
(***************************************************************************)
(* TLA+ model of the replication protocol documented in                   *)
(* designdocs/Replication.md.  Admits any implementation that satisfies   *)
(* the protocol's causal-ordering constraint on versions, including the   *)
(* Postgres reference implementation where transactions can commit out    *)
(* of order.                                                               *)
(*                                                                         *)
(* Each insert opens a transaction with a fresh version, then commits     *)
(* later (possibly after other transactions have committed).  Each row    *)
(* carries a watermark = min(in-flight versions at insert time) — the     *)
(* safe resume point for cursor wind-back.  Pull only sees committed rows *)
(* and advances the cursor to max(watermark) of the batch, so a late-     *)
(* committing low-version entry is picked up by a subsequent pull.        *)
(***************************************************************************)
