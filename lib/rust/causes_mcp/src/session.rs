...
mod tests {
    ...
    #[test]
    fn load_returns_none_for_empty_token() {
        let dir = test_dir("empty-token");
        let server = "http://localhost:50051";
        // Write a session file with an empty token.
        std::fs::create_dir_all(&dir).unwrap();
        let path = session_path(&dir, server);
        std::fs::write(&path, r#"{"session_token":""}"#).unwrap();

        let loaded = load(&dir, server).unwrap();
        assert!(loaded.is_none());
        std::fs::remove_dir_all(&dir).ok();
    }
