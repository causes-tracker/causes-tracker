...
- **Split from `api_db` to keep the AWS SDK off the SQL crate's compile graph.**
  The `sqlx_prepare` golden check recompiles `api_db` to validate query macros against the database; pulling in `aws-sdk-rds` + `aws-config` + their transitive crates roughly doubled that compile.
