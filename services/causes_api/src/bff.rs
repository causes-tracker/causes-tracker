...
///
/// Extraction is infallible: the inner `Option` is `None` when no cookie
/// is present (the user is not logged in).  Handlers check `session.0`
/// to distinguish authenticated from unauthenticated requests.
#[derive(Debug)]
pub(super) struct SessionToken(pub Option<String>);

impl<S: Send + Sync> axum::extract::FromRequestParts<S> for SessionToken {
    ...
    async fn from_request_parts(
        ...
        Ok(SessionToken(extract_session(&parts.headers)))
    }
}
...
> {
    ...
    Ok(AuthServiceClient::with_interceptor(
        channel,
        BearerInterceptor(token.to_string()),
    ))
}
...
mod tests {
    ...
    async fn session_token_extracts_from_cookie() {
        ...
        let token = SessionToken::from_request_parts(&mut parts, &())
            .await
            .unwrap();
        assert_eq!(token.0.as_deref(), Some("tok456"));
    }
